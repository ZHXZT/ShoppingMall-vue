...
export default {
  ...
  beforeMount() {
    //将query和params参数合并到searchparams中
  ...
  methods: {
    ...
    //面包屑分类的删除功能
    // 将属性设置成undefined后，该属性不会被传服务器，减轻压力
    removeCategoryName() {
      ...
      this.getData();
      //地址栏也要改
      //删除面包屑只是删除query参数，搜索框的params参数应该保留
    ...
    // 面包屑关键字的删除功能
    removeKeyword() {
      //给服务器的searchparams的keyword清空
      this.searchParams.keyword = undefined;
      this.getData();
      //通知兄弟组件header搜索框清楚关键子
      this.$bus.$emit("clear");
      //删除面包屑只是删除params参数，分类的query参数应该保留
      if (this.$route.query) {
...
//如果点击的是综合
  ...
  watch: {
    //监听路由信息有没有变化，变了就再次发请求
    $route(newValue, oldValue) {
      // 发请求前，先将路由的新参数给整理好
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      this.getData();
      //请求完后要把123级id清空，来接收下一次的123id参数
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
